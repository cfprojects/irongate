...
<div class="bluebox">
<div class="formblock">
<cfform id="formfield" action="index.cfm?action=updateSettings" method="post">
...
<div class="accordinghead">Update Settings</div>

<cfset box 	= 0>
<cfset line = 1>
<cfloop from="1" to="60" index="c">
<cfset i = listgetat(set,c,chr(10))>
	<cfoutput>
	<cfif left(i,2) eq '<!'>
...
<cfset line = 1+line>
	...
	<div style="padding-left:100px; padding-top:10px">
		<input title="Click Here to Save" class="btn btn-inverse btn-small" type="submit" value="Save" name="submit" />
	</div>
</div></div><br />
<br />
<cfelse>
	Missing irongate.cfm file. 
</cfif>
</cfform>

<cfquery name="chk" datasource="#irongate.datasource#" username="#irongate.username#" password="#irongate.password#">
	select count(errorid) as chk from #irongate.table#
</cfquery>
...
$("document").ready(function() {
	...
	<cftry>
	<cfif val(chk.chk)>
		$('.settingitem:contains("Compress Log Files")').css('display','none')
	</cfif>
	/////////////////////////////
	<cfset adminpath 	= "http://#cgi.HTTP_HOST##cgi.PATH_INFO#">
	<cfset adminpath	= ListDeleteAt(adminpath,listlen(adminpath,'\/'),'\/')>
	<cfset adminpath	= ListDeleteAt(adminpath,listlen(adminpath,'\/'),'\/')>
	<cfset adminpath	= "#adminpath#/administrator">
	var adminurl = $('.settingitem:contains("irongate Admin URL")').children().children('input')
	if ( !$(adminurl).val()) {
